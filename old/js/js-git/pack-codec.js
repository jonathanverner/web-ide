define(function (require, exports, module) {var inflateStream = require('./inflate-stream.js');
...
function encodePack(emit) {
  ...
  return function (item) {
    ...
    if (typeof item.num === "number") {
    ...
    else if (typeof item.type === "string" && binary.isBinary(item.body)) {
      ...
      if (!--left) {
        emit(binary.fromHex(sha1sum.digest()));
      }
    }
    else {
...
function packFrame(item) {
  ...
  var parts = [binary.fromArray(head)];
  if (typeof item.ref === "string") {
    parts.push(binary.fromHex(item.ref));
  }
